...
![dc locations](assets/images/dc-locations.png)

Основано на расположении ДЦ AWS ([2](https://docs.aws.amazon.com/global-infrastructure/latest/regions/aws-regions.html))

| Регионы | Расположение ДЦ | Пользователи | Failover ДЦ |
|--------|-----------------|--------------|-------------|
| США, Канада, Мексика, Бразилия | Северная Вирджиния, Огайо, Северная Калифорния | 174.1M | один из 2х других |
| Индия | Мумбаи, Хайдарабад | 100M | второй из пары |
| Европа, Великобритания, Средний Восток и Северная Африка | Франкфурт, Лондон | 115.5M | второй из пары |
| Юго-Восточная Азия, Япония, Австралия и Новая Зеландия | Сингапур | 41.8M | Мумбаи |

Всего 8 ДЦ, на каждый приходится не более 60M пользователей.
...
| `static.quora.com` | Статические ресурсы. | Средний, cacheable | 313 |
...
Т.к. используем K8s, серверами у нас будут поды. Скажем, используем AWS инстансы c5.2xlarge (8 vCPU, 16GB RAM) для нод кластера. В AWS существует готовое решение EKS для K8s кластеров, но использовать его здесь не будем.

Оценим, что инстанс c5.2xlarge как рабочая нода может поддерживать работу 16 подов под максимальной нагрузкой (каждый под около 0.5 vCPU и 0.75 Gi RAM). Учтем, что на ноду должен быть один Ingress под, остальных остается 15.

Всего у нас 32 пода => требуется минимум 32 / 15 = 2 рабочие ноды. Для надежности используем 4 нод, чтобы выдержать падение 2 из них.
...
Клиент аутентифицируется в auth-сервисе и получает пару refresh+access токенов. В последующие запросы к другим сервисам он включает короткоживущий access-токен. Когда access-токен истекает, клиент обновляет его в auth с помощью refresh-токена. Все остальные сервисы валидируют access-токен локально, не делая запросов в auth-сервис или БД. Тем самым, ускоряется авторизация и экономится место в БД, затрачиваемое иначе на хранение сессий.

### Сервисы

| Сервис | Роль | Связи |
|--------|------|-------|
| **Search** | Выполняет поиск по вопросам | Отправляет запрос в Elastic search |
| **Write** | Выполняет запись пользовательских данных (профилей, вопросов, ответов, лайков, подписок) | Записывает данные в кластер Postgres или Neo4j |
| **Auth** | Выдает пользователю refresh и access токены | Читает данные пользователей из Postgres, записывает отозванные токены в Redis |
| **Content** | Выдает содержимое профилей, вопросов, ответов, вычисляет и выдает ленту рекомендованных вопросов | Записывает основные данные в кластер Postgres или Neo4j, кеширует данные ленты в Redis |
| **Images** | Проверяет и сжимает изображения. Добавляет URL в Redis перед тем, как Write сервис запишет их вместе с answer в Postgres | Загружает изображения в кластер Minio |
| **Aggregation** | Асинхронно агрегирует пользовательские данные | Читает данные из расчетных БД и записывает в шарды основных БД |

## 11. Расчет ресурсов

### Сервисы в оркестрации

| Сервис | Целевая пиковая нагрузка на контейнер | CPU/r | CPU/l | RAM/r | RAM/l | Cnt |
|--------|---------------------------------------|-------|-------|-------|-------|-----|
| **Search** | 35 RPS | 0.3 | 0.5 | 0.5 Gi | 1 Gi | 4 |
| **Write** | 13 RPS | 0.3 | 0.5 | 0.5 Gi | 1 Gi | 3 |
| **Auth** | 35 RPS | 0.5 | 0.7 | 0.5 Gi | 1 Gi | 4 |
| **Content** | 162 RPS | 1 | 1.5 | 1 Gi | 2 Gi | 10 |
| **Images** | 140 RPS | 0.5 | 0.7 | 1 Gi | 2 Gi | 8 |
| **Aggregation**| - | 0.5 | 0.7 | 1 Gi | 2 Gi | 3 |
| Сумма | 385 RPS | 19.6 | 29 | 26.5 Gi | 53 Gi | 32 |

В пункте "Локальная балансировка" было рассчитано минимально 2 ноды. Однако с текущими расчетами выходит 4.

На 4 ноды требуется до 7.25 vCPU и 13.25 Gi RAM на ноду. Подходит c5.2xlarge.
...
| **Neo4j** | ~60 ГБ | 1 | 3 | 8 | 32 ГБ | db.t4g.2xlarge |
| **Redis** | ~50 ГБ | 1 | 3 | 4 | 16 ГБ | cache.t4g.large |

### Kubernetes

| Роль | Кол-во | vCPU на ноду | RAM на ноду | Тип инстанса AWS |
|------|--------|--------------|-------------|------------------|
| **K8s Control Plane** | 5 | 2 | 8 Gi | m5.large |
| **K8s Worker Nodes** | 7 | 8 | 16 Gi | c5.2xlarge |

Возьмем 7 воркер-нод для надежности.
