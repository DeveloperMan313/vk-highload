...
Количество шардов рассчитано так, чтобы на каждый шард PostgreSQL приходилось не более 100Гб данных.
...
|---------|--------------------|---------------|-------------------|------------|
| **user** | Hash | hash(user_id) | 16 | Master-Slave (2 реплики) |
| **question** | Composite Hash | hash(question_id + topic_id) | 2 | Master-Slave (2 реплики) |
| **topic** | No sharding | - | 1 | Master-Slave (2 реплики) |
| **question_topic** | Composite Hash | hash(question_id + topic_id) | 4 | Master-Slave (2 реплики) |
| **answer** | Composite Hash | hash(user_id + question_id) | 8 | Master-Slave (2 реплики) |
| **answer_vote** | Composite Hash | hash(user_id + answer_id) | 1 | Neo4j Causal Cluster |
| **user_sub_user** | Composite Hash | hash(user_id + subbed_to_user_id) | 1 | Neo4j Causal Cluster |
| **user_sub_topic** | Composite Hash | hash(user_id + topic_id) | 1 | Neo4j Causal Cluster |
| Отозванные токены и ленты рекомендаций | No sharding | - | 1 | Master-Slave (2 реплики) |
